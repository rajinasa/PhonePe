...
def brd_all_states(state):
    br=Aggre_user[["States", "Brands", "Transaction_count"]]
    br1=br.loc[(Aggre_user["States"]==state)]
    br2= br1[["Brands", "Transaction_count"]]
    br3= br2.groupby("Brands")["Transaction_count"].sum()
    br4= pd.DataFrame(br3).reset_index()
    fig_bu= px.bar(br4, x= "Brands", y= "Transaction_count", title= "BRAND and TRANSACTION COUNT",
                color_discrete_sequence=px.colors.sequential.Bluered_r)
    fig_bu.update_layout(width= 1000, height= 500)
    return st.plotly_chart(fig_bu)
...
def agg_user(sel_year,state):
    year=int(sel_year)
    brd=Aggre_user[["States", "Years", "Brands", "Transaction_count"]]
    brd1=brd.loc[(Aggre_user["States"]==state)&(Aggre_user["Years"]==year)]
    brd2=brd1.groupby("Brands")["Transaction_count"].sum()
    brd3=pd.DataFrame(brd2).reset_index()

    fig_brd=px.bar(brd3, x= "Brands", y="Transaction_count", title="BRANDS and TRANSACTION COUNT",
                    color_discrete_sequence=px.colors.sequential.Cividis_r)
    fig_brd.update_layout(width= 600, height= 500)
...
with tab1:
    st.header("PHONEPE - INDIA'S BEST TRANSACTION APP")
